...
美化日志输出格式命令:  `git log [--pretty=oneline]` 

![image_21283](assets/image_21283.png)

#### 4.5.6 git reflog

git reflog 命令的作用是查看提交日志

![image_26653](assets/image_26653.png)

通过git reflog 命令查看日志，可以发现每次都会提交都会产生一个版本号，并且通过 `HEAD@{数字}` 显示回到当前这个历史版本需要走多少步。

#### 4.5.7 git log --oneline 

git  log [--oneline] 命名的作用是查看提交日志

![image_28656](assets/image_28656.png)
...
`git diff [fileName]` 将工作区中的文件和暂存区中文件进行比较

![image_28335](assets/image_28335.png)



`git diff [HEAD] [fileName]` 将暂存区和本地库中的文件进行比较 

![image_9883](assets/image_9883.png)
