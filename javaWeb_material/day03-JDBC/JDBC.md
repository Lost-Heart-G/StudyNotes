...
* 获取数据库连接
  ...
  * `static Connection getConnection(String url, String user, String password)` :  尝试建立给定数据库URL的连接
  ...
  * url ： 连接路径
    ...
    > * 配置 useUnicode=true 参数，使用Unicode编码方式
    > * 配置 characterEncoding=UTF-8 参数, 使用UTF-8编码
    > * 配置 serverTimezone=Asia/Shanghai 参数，使用东八区时区
...
* 回滚事务
  ...
  * `void rollback() `  撤销当前事务中所有的更改，并释放此Connection对象所持有的所有数据库锁
...
使用 `preparedStatement.addBatch()` 将修改放入一个批次中

使用 `preparedStatement.executeBatch()` 执行批处理

使用 `preparedStatement.clearBatch()`  清除批处理中的数据

> 注意:  需要设置批处理开启 `&rewriteBatchedStatements=true` 同时需要开启 `useServerPrepStmts=true` 和 `cachePrepStmts=true`   
>
...
public class TestTransaction2 {
    private static String driver ="com.mysql.cj.jdbc.Driver";
    private static String url="jdbc:mysql://127.0.0.1:3306/mydb?useSSL=false&useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useServerPrepStmts=true&cachePrepStmts=true&&rewriteBatchedStatements=true";
    private static String user="root";
    private static String password="root";
    public static void main(String[] args) {
        testAddBatch();
...
* 执行DDL、DML语句
  ...
  * `int excuteUpate(String sql)` : 执行给定的SQL语句，这可能是 INSERT, UPDATE, 或 DELETE语句，或者不返回任何内容，如SQL DDL语句的SQL语句。
...
* 开启预编译功能
  ...
  ```sql
  useServerPrepStmts=true  是否开启预编译
  cachePrepStmts=true  是否启用预编译缓存
  ```
