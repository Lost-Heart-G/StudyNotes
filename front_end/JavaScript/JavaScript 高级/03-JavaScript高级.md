...
概念：函数与函数相互嵌套的情况，将内函数保存到外部，能从外部访问内部函数环境下的所有变量
...
| ()              | 从两个直接量中选择一个，分组   eg:  gr(a\|e)y      匹配gray和grey |
| [\u4e00-\u9fa5] | 匹配汉字                                                     |
...
* `reg.test(str)`         是否匹配成功。返回 true 和 false
