...
伪元素选择器 本质上是 插入一个元素( 标签  盒子) 只不过是行内元素  和 span a 标签等相似
    ...
    <style>
        *{
        ...
        li a {
            ...
            background: url(media/lTcb_ve.png) no-repeat;
            padding-left:15px;
            color: #fff;
        ...
        li a span{
            display: inline-block;
            height: 33px;
            background: url(media/lTcb_ve.png) no-repeat right;
            /*span  不能给宽度  利用 padding 挤开 我要span右边的圆角 所以 背景位置 右对齐*/
            padding-right: 15px;
...
1. `flex`子项目在主轴的缩放比例， 不指定 flex 属性，则不参与伸缩分配
   ...
   ​    max-width   最大宽度   `max-width: 1800px;` 最大宽度  不能大于 1800px; 

2. `flex-direction`  调整主轴方向（默认为水平方向） **给父盒子添加**
...
| 值           | 描述                                                         |
| ------------ | ------------------------------------------------------------ |
| nowrap       | 默认值。规定灵活的项目不拆分或不拆列。不换行，则收缩(压缩) 显示 |
...
*/
flex-flow: colum wrap;  /* 两者的综合 */
```

7. `align-content`    堆栈（由flex-wrap产生的独立行）多行垂直对齐
   ...
   | flex-end     | 项目位于容器的结尾                               |      |
   | space-betwen | 项目位于各行之间留有空白的容器内。               |      |
   | space-around | 项目位于各行之前、之间、之后都留有空白的容器内。 |      |

8. `order`      控制子项目的排列顺序，正序方式排序，从小到大

用 css 来控制盒子的前后顺序。用 order 
...
`transition-duration`        花费时间  单位：秒 (s)    比如 0.5s  这个单位必须写

`transition-timing-function`    运动曲线  默认 **ease** 

`transition-delay`           何时开始   默认 0s 立刻开始
...
div{
    ...
    /*先改变宽度 再改变高度*/
    /*transition 过渡的意思  这句话写在div 里面而不是 hover 里面*/ 
}
div:hover{  /* 鼠标经过盒子 我们的宽度改变 */
    width: 600px;
    height: 300px;
}

transition: all 0.6s;  /* 所有属性都变化  用 all 就可以了  后面俩属性可以省略*/
```
...
div{
    ...
    height: 200px;
    background-color : pink;
 	/* tanslate 移动的距离 如果是 % 不是以父级宽度为准， 以自己的宽度为准 */
	transform: translate(50%); /* 意思就是 走 div 盒子 自己宽度 200 的一半 走 100像素*/	   
}
```

使用 `translate` 方法来将文字或图像在水平方向和垂直方向上分别垂直移动50像素。

可以改变元素的位置，x、y可为负值；

* `translate(x, y)`    水平方向和垂直方向同时移动（也就是 X 轴和 Y 轴同时移动）
* `translate(x)`           仅水平方向移动（ X 轴移动）
* `translate(Y)`           仅垂直方向移动（ Y 轴移动）
...
* `scale(X)`            元素仅水平方向缩放（ X 轴缩放）
* `scale(Y)`            元素仅垂直方向缩放（Y轴缩放）           ）
...
如果是4个角，可以用 left top 这些，如果想要精确位置，可以用 px 像素。

```css
/* 改变元素到原点 x 为10 y 为 10， 然后进行顺时针旋转45度 */
    ...
    <style>
        *{
        ...
        div img{
            ...
            tranform-origin: top right;  /* 调整元素转换变形的原点 */
        }
        div:hover img:nth-child(1){
            transform: rotate(60deg);  /* 2D 旋转 
            */
        }
        div:hover img:nth-child(1){
            transform: rotate(120deg);
...
该实例通过 `skew()` 方法把元素水平方向上倾斜30度，垂直方向保持不变。
...
> `rotateY()` 引起的旋转量由 angle 指定。如果为正，则顺时针方向移动；如果为负，则逆时针方向移动。
>
> angle: CSS 数据类型   用于表示角度大小
...
| ------------------------- | ------------------------------------------------------ | ---- |
| @keyframes                | 规定动画                                               | 3    |
| animation                 | 所有动画属性的简写属性，除了 animation-paly-state 属性 | 3    |
| animation-name            | 规定 @keyframes  动画的名称                            | 3    |
| animation-duration        | 规定动画完成一个周期所花费的秒或毫秒。默认是 0         | 3    |
| animation-timing-function | 规定动画的速度曲线。默认是"ease"                       | 3    |
| animation-delay           | 规定动画何时开始。默认是 0                             | 3    |
| animation-iteration-count | 规定动画被播放的次数，默认是 1                         | 3    |
| animation-direction       | 规定动画是否在下一个周期逆向播放。默认是"normal"       | 3    |
